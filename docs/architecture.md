...
| 5 | **Reuse `hw.*` for generic hardware** | Fans, PSUs, and generic platform temp/voltage sensors stay in [`hw.*`](https://opentelemetry.io/docs/specs/semconv/attributes-registry/hw/). Use `network.*` only when the object carries forwarding/topology semantics — the one environmental exception is the **transceiver-DOM diagnostic set** (module temperature/Vcc), which lives in `network.optical.*` with the optical signal it is read alongside, split from `hw.*` by source (a transceiver-DOM leaf vs a generic platform sensor). |
